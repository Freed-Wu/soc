 ...
 * @author Wang Yang
 *
 * axi-tangxi内核接口文件，这包含IOCTL接口定义。这是从用户空间到AXI TANGXI设备的接口。
 *
 * @bug
 **/
...
#define NETWORK_ACC_CONFIG                      _IOR(AXITANGXI_IOCTL_MAGIC, 2, \
                                                        struct network_acc_reg)

#define NETWORK_ACC_START                       _IO(AXITANGXI_IOCTL_MAGIC, 3 )

#define NETWORK_ACC_GET                         _IOR(AXITANGXI_IOCTL_MAGIC, 4, \
                                                       struct network_acc_reg)

#define PSDDR_TO_PLDDR                          _IOR(AXITANGXI_IOCTL_MAGIC, 5, \
                                                        struct axitangxi_transaction)

#define PLDDR_TO_PSDDR                          _IOR(AXITANGXI_IOCTL_MAGIC, 6, \
                                                        struct axitangxi_transaction)
