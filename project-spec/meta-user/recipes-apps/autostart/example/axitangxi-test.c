...
//         {
//             count++;
//             printf("different data! : tx address 0x%x: %u; rx address 0x%x:
...
int main(int argc, char **argv) {
  ...
  if (axitangxi_dev == NULL) {
    ...
    printf("axitangxi_dev_init success\n");
  }
  ...
  if (weight_tx_buffer == NULL) {
    ...
    printf("axitangxi_malloc weight_tx_buffer success\n");
  }
  ...
  if (weight_rx_buffer == NULL) {
    ...
    printf("axitangxi_malloc weight_rx_buffer success\n");
  }
  ...
  if (network_acc_dev == NULL) {
    ...
    printf("network_acc_dev_init success\n");
  }
  ...
  if (rc != 0) {
    ...
    printf("network_acc_data_init success\n");
  }
  // 网络加速器配置
  rc = acc_config_test(network_acc_dev);
  if (rc != 0) {
    printf("Failed to acc_config_test\n");
    goto ret;
  } else {
    printf("acc_config_test success\n");
  }
  // 网络加速器开始运行
  rc = acc_start_test(network_acc_dev);
  if (rc != 0) {
    printf("Failed to acc_start_test\n");
    goto ret;
  } else {
    printf("acc_start_test success\n");
  }

  // 网络加速器获取结果（与start信号同时执行，不要加断点）
  rc = acc_get_test(network_acc_dev);
  if (rc != 0) {
    printf("Failed to acc_get_test\n");
    goto ret;
  } else {
    printf("acc_get_test success\n");
  }
