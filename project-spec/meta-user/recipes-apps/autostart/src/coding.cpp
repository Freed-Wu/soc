...
extern "C" size_t coding(gmm_t* gmm,uint16_t* trans_addr,size_t trans_len,uint8_t* data_addr,uint32_t low_bound,uint32_t high_bound,uint32_t freqs_resolution){
	...
	for(size_t i=0;i<trans_len;i++,gmm++){
		...
		uint32_t total_freqs=freqs_table.total_freqs,symlow=freqs_table.sym_low,symhigh=freqs_table.sym_high;
...
double test_one(char *input_path, char *bin_path, bool is_bin = false ) {
  ...
  clock_t start, end;
  start = clock();
  int bin_len1 = coding (gmm1, enc_data, data_len, data1, data_min, data_max, freqs_resolution); 
  end = clock();
  printf("  encode_time: %f\n", (double)(end - start) / CLOCKS_PER_SEC);

  if (is_bin) write_bin(data1,bin_len1,bin_path);
