...
void ArithmeticCoderBase::update(const GmmTable &freqs, uint32_t symbol) {
  ...
  // While low's top two bits are 01 and high's are 10, delete the second
  // highest bit of both
  while ((low & ~high & quarterRange) != 0) {
    underflow();
    low = (low << 1) ^ halfRange;
...
uint32_t ArithmeticDecoder::read(const GmmTable &freqs) {
  ...
  uint64_t offset = code - low; // 200-100=100
  uint64_t value =
      ((offset + 1) * total - 1) / range; // eg 100 100/288 A:90-101 B:102-110
  if (value * range / total > offset)
    throw std::logic_error("Assertion error");
  if (value >= total)
    throw std::logic_error("Assertion error");
  ...
  uint32_t symbol = start;
  if (!(freqs.symlow[symbol] * range / total <= offset &&
        offset < freqs.symhigh[symbol] * range / total))
    throw std::logic_error("Assertion error");
  update(freqs, symbol);
  if (!(low <= code && code <= high))
    throw std::logic_error("Assertion error: Code out of range");
