...
int softmax(int probs[3],uint16_t exp_table[],int scale=1000,int x_bound=-12){
...
void EncTable::get_bound(int x) {
	...
	l_bound = 0, r_bound = 0;
	for (int i = 0; i < 3; i++) {
		int idx_l = (long long)((low_bound * scale_pred - means[i]) - scale_pred / 2) * cdf_scale / stds[i];
		idx_l = clamp(idx_l, -base, base);
		idx_l += base;

		int idx_r = (long long)((high_bound * scale_pred - means[i]) + scale_pred / 2) * cdf_scale / stds[i];
	...
	for (int i = 0; i < 3; i++) {
		int idx_l = (long long)((x * scale_pred - means[i]) - scale_pred / 2) * cdf_scale / stds[i];
		idx_l = clamp(idx_l, -base, base);
		idx_l += base;

		int idx_r = (long long)((x * scale_pred - means[i]) + scale_pred / 2) * cdf_scale / stds[i];
		idx_r = clamp(idx_r, -base, base);
		idx_r += base;
