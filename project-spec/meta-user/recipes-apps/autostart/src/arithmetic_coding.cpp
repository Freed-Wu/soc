...
int softmax(int probs[3], uint16_t scale_pred, uint16_t exp_table[],
            int scale = 1000, int x_bound = -12) {
...
void EncTable::get_bound(int x) {
  ...
  for (int i = 0; i < 3; i++) {
    int64_t idx_l = (long long)((x * scale_pred - means[i]) - scale_pred / 2) *
                    cdf_scale / stds[i];
    idx_l = clamp(idx_l, -base, base);
    idx_l += base;

    int64_t idx_r = (long long)((x * scale_pred - means[i]) + scale_pred / 2) *
                    cdf_scale / stds[i];
    idx_r = clamp(idx_r, -base, base);
    idx_r += base;
