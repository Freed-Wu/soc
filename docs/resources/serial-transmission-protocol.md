...
| -------  | ------         | ---  | ---------------------                                                                                                                                                              | --------------             |
| 1~4      | 帧头           | 4    | 0x3A62043F                                                                                                                                                                         | -                          |
| 5~7      | 帧计数         | 3    | 从0往上计数                                                                                                                                                                        | 除开机外，计数器不清零     |
| 8        | 标志字         | 1    | 相机1摄像模式：0x8A(H.265)                                                                                                                                                         | -                          |
| 8        | 标志字         | 1    | 相机1照相模式：0x8C(JPEG)                                                                                                                                                          | -                          |
| 8        | 标志字         | 1    | 相机1照相模式：0x8E(YUV420)                                                                                                                                                        | -                          |
| 8        | 标志字         | 1    | 相机2摄像模式：0x9A(H.265)                                                                                                                                                         | -                          |
| 8        | 标志字         | 1    | 相机2照相模式：0x9C(JPEG)                                                                                                                                                          | -                          |
| 8        | 标志字         | 1    | 相机2照相模式：0x9E(YUV420)                                                                                                                                                        | -                          |
| 8        | 标志字         | 1    | 搭载板回传数据：0xAA                                                                                                                                                               | -                          |
| 9~12     | 数传文件号     | 4    | -                                                                                                                                                                                  | -                          |
| 13~14    | 帧内包号       | 2    | （1）有且只有1包：0x0000；                                                                                                                                                         | -                          |
| 13~14    | 帧内包号       | 2    | （2）多于1包的首包：0x0001；                                                                                                                                                       | -                          |
| 13~14    | 帧内包号       | 2    | （2）尾包：0xFFFF；                                                                                                                                                                | -                          |
| 13~14    | 帧内包号       | 2    | 其他顺序编号。                                                                                                                                                                     | -                          |
...
| 21~532    | 压缩数据 | 512  |                                                                                                                                                                                    | 前 n2 个字节为有效压缩数据，有效数据不足512字节的补零  |
| 533~534  | CRC校验        | 2    | 生成多项式为： $G(X) = X^16+X^12+X^5+1$ 寄存器初相设为全 1 | CRC-16                     |

说明：

- 表中一字节为8bit；
- 数据输出时，每个域的高字节在前，低字节在后；字节内高位B7在前，低位B0在后。例如：帧计数3个字节，先输出最高字节，最后输出最低字节。帧头4字节，依次输出为：0x3A、0x62、0x04、0x3F
...
| 2   | 查询01H          | 指令类型         | 1字节 |
| 3   | 0x0000XXXX     | 完成状态返回数据文件帧数 | 4字节 |
| 3   | 0x0000XXXX     | 接收状态返回成功接收帧数 | 4字节 |
| 3   | 0x0000XXXX     | 处理状态则表示已成功接收 | 4字节 |
| 3   | 接收状态0xAAAAAAAA | 搭载板工作状态      | 4字节 |
| 3   | 完成状态0x55555555 | 搭载板工作状态      | 4字节 |
| 3   | 处理状态0xAAAA5555 | 搭载板工作状态      | 4字节 |
| 4   | 0xXXXX         | CRC校验        | 2字节 |
